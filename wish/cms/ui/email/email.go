...
func View(m Model) string {
	s := "You don't have to provide us with an email address.\n"
	s += "However, an email address provides you with two primary benefits:\n"
	s += "- Fallback when you lose your keypair\n"
	s += "- Ability to receive email digests for feeds.sh\n\n"
...
func setName(m Model) tea.Cmd {
	return func() tea.Msg {
		valid, err := m.dbpool.ValidateEmail(m.newName)
		...
		err = m.dbpool.SetEmail(m.user.ID, m.newName)
		if err != nil {
			return errMsg{err}
		}
