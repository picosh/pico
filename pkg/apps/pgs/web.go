...
import (
	...
	"github.com/picosh/pico/pkg/http_cache"
...
func NewPromCacheMetrics(reg prometheus.Registerer) *PromCacheMetrics {
	...
	return &PromCacheMetrics{
		...
		CacheSize: auto.NewGauge(prometheus.GaugeOpts{
			Namespace: name,
			Subsystem: "http_cache",
			Name:      "total_size",
...
func NewPgsHttpCache(cfg *PgsConfig, upstream http.Handler) *http_cache.HttpCache {
	ttl := time.Minute * 10
	metrics := NewPromCacheMetrics(prometheus.DefaultRegisterer)
	cache := expirable.NewLRU(0, metrics.EvictCacheItem, ttl)
	httpCache := &http_cache.HttpCache{
		Ttl:      ttl,
		Logger:   cfg.Logger,
		Upstream: upstream,
		Cache:    cache,
	...
	httpCache.Logger.Info("http_cache initiated", "ttl", httpCache.Ttl, "storage", "lru")
	return httpCache
}
...
func CacheMgmt(ctx context.Context, notify chan string, cfg *PgsConfig, cacher http_cache.Cacher) {
